...
jQuery(document).ready(function($){
    ...
    jQuery(document).ready(function($){
    	$("#form-rastreo").submit(function() {
    		event.preventDefault();
    		$valor_cliente = $("#cliente").val();
    		$valor_pedido = $("#pedido").val();
    		$robot = $("#custom").val();
    		//$('#form-rastreo').attr('action', 'https://prontoform.dico.com.mx/rastreo.html');
    		if ($valor_cliente=="" || $valor_cliente == null || $("#cliente").length==0 || !/^([0-9])*$/.test($valor_cliente)) {
    			$(".label-required-cliente").show();
    			return false;	
    		}
    		else if( $valor_pedido=="" || $("#pedido").length==0)
    		{
    			$(".label-required-cliente").hide();
    			$(".label-required-pedido").show();
    			return false;
    		}
    		else if( $robot!="")
    		{
    			console.log("Robot a la vista");
    			return false;
    		}
    		else{
    			$("#form-container").hide();
    		$iframe = '<iframe src="https://prontoform.dico.com.mx/rastreo.html?cliente='+$valor_cliente+'&pedido='+$valor_pedido+'" frameborder=0 width="691px" height="1100px" scrolling="no"> </iframe>'
    		$("#iframe-rastreo").html($iframe);
    			$(".label-required-cliente").hide();
    			$(".label-required-pedido").hide();
    			}	
		...
		$("#pedido").focusout(function(){
    		$valor_pedido = $("#pedido").val();
    		if ($valor_pedido=="" || $valor_pedido == null || $("#pedido").length==0) {
    			$(".label-required-pedido").show();
    		}
    		else{
    			$(".label-required-pedido").hide();
