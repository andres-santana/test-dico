...
if ($file) {
  foreach ($file as $line) {
    ...
    if($_product && $tiempo_entrega!=0){
      ...
      Mage::getResourceSingleton('catalog/product_action')->updateAttributes(array($_product->getId()), $attrData, 19);
      echo "Producto actualizado [SKU][".$sku."] - [Tiempo de entrega][".$tiempo_entrega."]"."<br>";  
      //$log = "Producto actualizado [SKU][".$sku."] - [Tiempo de entrega][".$tiempo_entrega."]"."<br>";  
      $log = new Log("tiempoEntrega", "./logs/");
      echo $log->insert("[SKU][".$sku."] - [Tiempo de entrega][".$tiempo_entrega."] - [Portal][Lomas Baby]", false, true, false, "LomasBaby");
    }
    else{
