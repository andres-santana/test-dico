...
{
	class Int_Advancedpermissions_Block_Report_Review_Product_Grid extends Mage_Adminhtml_Block_Report_Review_Product_Grid
	{
	}

} else {
	 
	class Int_Advancedpermissions_Block_Report_Review_Product_Grid extends Mage_Adminhtml_Block_Widget_Grid
	{
		...
		protected function _prepareCollection()
		{
			...
			if($userType != NULL){
				...
				$newCollection 		= Mage::getResourceModel('reports/review_product_collection')->joinReview();
				$userReviewProSku 	= '';
				foreach($newCollection as $_newCollection){
					$proSku 			= $_newCollection->getSku();
					$proId  			= Mage::getModel('catalog/product')->loadByAttribute('sku', $proSku)->getId();
					
					$reviewCollection 	= Mage::getModel('review/review')
											->getCollection()
											->addFieldToFilter('entity_pk_value', $proId);
					
					if(count($reviewCollection) > 0){
						foreach($reviewCollection as $_reviewCollection){
						
							$storeId = Mage::getModel('review/review')
										->load($_reviewCollection->getId())
										->getStoreId();
										
							if(in_array($storeId, $userObj->getCurrentUserStoreView())){
								$userReviewProSku[] = Mage::getModel('catalog/product')
														->load($_reviewCollection->getEntityPkValue())
														->getSku();
				...
				if(count($userReviewProSku) > 0){
					$collection->addFieldToFilter('sku', array_unique($userReviewProSku));
				}
