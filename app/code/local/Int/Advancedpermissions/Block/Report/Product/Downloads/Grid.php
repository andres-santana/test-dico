...
if(Mage::getStoreConfig('advancedpermissions/settings/enabled') == '0')
{
	...
	class Int_Advancedpermissions_Block_Report_Product_Downloads_Grid extends Mage_Adminhtml_Block_Report_Product_Downloads_Grid
	{
	...
	class Int_Advancedpermissions_Block_Report_Product_Downloads_Grid extends Mage_Adminhtml_Block_Widget_Grid
	{
		...
		protected function _prepareCollection()
		{
			...
			if($userType != NULL){
				...
				if($storeId){
					if(is_array($storeId) == false){ $storeId = array($storeId); }
					$storeId = array_intersect($storeId, $userObj->getCurrentUserStoreView());
				} else {
					$storeId = $userObj->getCurrentUserStoreView();
				}
			}
			*/
			var_dump($storeId);
			...
			die();*/
			//$collection->addAttributeToFilter('store_id', array(2));
