...
if(Mage::getStoreConfig('advancedpermissions/settings/enabled') == '0')
{
	...
	class Int_Advancedpermissions_Block_Report_Product_Lowstock_Grid extends Mage_Adminhtml_Block_Report_Product_Lowstock_Grid
	{
	...
	class Int_Advancedpermissions_Block_Report_Product_Lowstock_Grid extends Mage_Adminhtml_Block_Widget_Grid
	{
	    ...
	    protected function _prepareCollection()
	    {
			...
			if($userType != NULL){
				...
				if($storeId == NULL){
					...
					if(count($userObj->getCurrentUserStoreView()) > 0){
						
						foreach($userObj->getCurrentUserStoreView() as $userStoreId){
							
							$newCollection = Mage::getResourceModel('reports/product_lowstock_collection')
									            ->addAttributeToSelect('*')
									            ->setStoreId($storeId)
									            ->filterByIsQtyProductTypes()
									            ->joinInventoryItem('qty')
							...
							$newCollection->addStoreFilter($userStoreId);
							
							if(count($newCollection) > 0){
								foreach($newCollection as $productDetails){
									$userProductIds[] = $productDetails->getId();
								}
							}
								
						}
						
						$collection->addIdFilter(array_unique($userProductIds));							
