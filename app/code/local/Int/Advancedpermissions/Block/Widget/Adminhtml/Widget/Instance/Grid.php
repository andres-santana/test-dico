...
{
	class Int_Advancedpermissions_Block_Widget_Adminhtml_Widget_Instance_Grid extends Mage_Widget_Block_Adminhtml_Widget_Instance_Grid
	{
	...
	class Int_Advancedpermissions_Block_Widget_Adminhtml_Widget_Instance_Grid extends Mage_Adminhtml_Block_Widget_Grid
	{
		/**
		...
		protected function _prepareCollection()
		{
			...
			if($userType != NULL){
				...
				$userStoreResult = $dbConnection->query("SELECT `storeview_id` FROM ".$userCategoryTable." WHERE `user_id`='".$userId."'");
				$userStoreRows	= $userStoreResult->fetchAll(PDO::FETCH_ASSOC);
				if(count($userStoreRows) > 0){
					unset($_arrStoreIds);
					foreach($userStoreRows as $userStore){
						$arrStoreIds = explode(',', $userStore['storeview_id']);	
						foreach($arrStoreIds as $storeIds){
							$_arrStoreIds[] = $storeIds;
						}
					}
					$collection->addStoreFilter($_arrStoreIds);
				}
			}
