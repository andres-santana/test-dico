 ...
 */
class Int_Advancedpermissions_Block_Widget_Grid_Column_Filter_Store extends Mage_Adminhtml_Block_Widget_Grid_Column_Filter_Store
{
    ...
    public function getHtml()
    {
		...
		if($userType != NULL){
			...
			$result = $dbConnection->query("SELECT `websiteid`,`store_ids` FROM ".$userWebsiteTable." WHERE `userid`='".$userId."'");
			$rows = $result->fetchAll(PDO::FETCH_ASSOC);
			foreach($rows as $row){
				$userWebIds[] = $row['websiteid'];
				if(substr_count($row['store_ids'], ",") > 0){
					$storeIds = explode(",", $row['store_ids']);
					foreach($storeIds as $storeId){ $userStoreIds[] = $storeId; }
				} else {
					$userStoreIds[] = $row['store_ids'];
				}
			}
		}
        ...
        foreach ($websiteCollection as $website) {
            $websiteShow = false;
			if($userType != NULL){				
				if(in_array($website->getId(), $userWebIds)){
				
					foreach ($groupCollection as $group) {
						
						if(in_array($group->getId(), $userStoreIds)){
