...
{
	class Int_Advancedpermissions_Block_Tag_Tag_Grid extends Mage_Adminhtml_Block_Tag_Tag_Grid
	{
	...
	class Int_Advancedpermissions_Block_Tag_Tag_Grid extends Mage_Adminhtml_Block_Widget_Grid
	{
		/**
		 * Constructor
		 *
		...
		protected function _prepareCollection()
		{
			...
			if($userType != NULL){
				...
				$newCollection	= Mage::getResourceModel('tag/tag_collection')
										->addSummary(Mage::app()->getStore()->getId())
										->addStoresVisibility();
				$userTagIds	= '';
				foreach($newCollection as $newData){			
					$tagStores = array_intersect($newData->getStores(), $userObj->getCurrentUserStoreView());
					if(count($tagStores) > 0){ $userTagIds[] = $newData->getId(); }
				}
				$collection->addFieldToFilter('tag_id', $userTagIds);
			}
