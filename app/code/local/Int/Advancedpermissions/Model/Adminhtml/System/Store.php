 ...
 */
 if(Mage::getStoreConfig('advancedpermissions/settings/enabled') == '0'){
	...
	class Int_Advancedpermissions_Model_Adminhtml_System_Store extends Mage_Adminhtml_Model_System_Store
	{
	...
	class Int_Advancedpermissions_Model_Adminhtml_System_Store extends Varien_Object
	{
		...
		protected function _loadWebsiteCollection()
		{
			...
			if($userType == NULL){
				...
				$this->_websiteCollection = Mage::getModel('core/website')->getCollection()->addIdFilter($userObj->getCurrentUserWebsite());
			}
			return $this;
		}
		...
		protected function _loadGroupCollection()
		{
			...
			if($userType == NULL){
				...
				foreach (Mage::app()->getWebsites() as $website) {
					foreach ($website->getGroups() as $group) {
						if(in_array($group->getId(), $userObj->getCurrentUserStore())){
							$this->_groupCollection[$group->getId()] = $group;
						}
					}
				}
		...
		protected function _loadStoreCollection()
		{
			...
			if($userType == NULL){
				...
				$userObj = new Int_Advancedpermissions_Block_Catalog_Product_Grid();
				$this->_storeCollection = Mage::getModel('core/store')->getCollection()->addIdFilter($userObj->getCurrentUserStoreView());
			}
