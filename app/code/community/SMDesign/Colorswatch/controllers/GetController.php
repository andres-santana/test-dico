...
class SMDesign_ColorswatchProductView_GetController extends Mage_Core_Controller_Front_Action {
	...
	function mainImageAction() {
    	...
    	$imageSelector = $this->getRequest()->getParam('image_selector', '.cloud-zoom img');
		...
		/* CloudZoom Mode Init */
		$magicZoomOutputHelper = Mage::helper('catalog/output');
		$magicZoomHelper = Mage::helper('shoppersettings/image');
    	$magicZoomConfig = Mage::getStoreConfig('shoppersettings');

		list($mainImageWidth, $mainImageHeight) = $magicZoomHelper->getMainSize();
		if ( $mainImageWidth > 800 ) {
			$mainImageWidth = 800;
			$mainImageHeight = $magicZoomHelper->calculateHeight($mainImageWidth);
		}
		list($thumbImageWidth, $thumbImageHeight) = $magicZoomHelper->getThumbSize();
		
		$bigImageWidth 	= $mainImageWidth*2;
 		$bigImageHeight = $mainImageHeight*2;
		
    	/* end CloudZoom Mode Init */
		$images = array();
		if (count($products) > 0) {
...
jQuery('.cloud-zoom').attr('href','<?php echo $images[0]['big_image']?>');
jQuery('<?php echo $imageSelector ?>').attr('src','<?php echo $images[0]['image']?>');
...
<?php foreach ($images as $key=>$image) : ?>
	...
	<?php else : ?>
		$(li).update("<a href=\"<?php echo  $image['big_image']; ?>\" class=\"cloud-zoom-gallery\" title=\"<?php echo  $image['label'] ?>\" rel=\"useZoom: 'cloud_zoom', smallImage:'<?php echo  $image['image']; ?>'\"><img src=\"<?php echo  $image['thumb'] ?>\" data-srcX2=\"<?php echo  $image['thumb'] ?>\" width=\"<?php echo $thumbImageWidth; ?>\" height=\"<?php echo $thumbImageHeight; ?>\" alt=\"<?php echo  $image['label'] ?>\" /></a>");
		<?php // $(li).update("<a href=\"#\" onclick=\"popWin('<?php echo Mage::getUrl('catalog/product/gallery', array('id'=>$image['product_id'], 'image'=>$image['id'], 'pid'=>$productId)) ?>', 'gallery', 'width=300,height=300,left=0,top=0,location=no,status=yes,scrollbars=yes,resizable=yes'); return false;\" title=\"<?php echo  $image['label'] ?>\"><img src=\"<?php echo  $image['thumb'] ?>\" width=\"56\" height=\"56\" alt=\"<?php echo  $image['label'] ?>\" /></a>"); ?>
	<?php endif; ?>
...
for (var i = 1; i <= myCarousel.options.size; i++) {
	var swap = myCarousel.get(i);
	myCarousel.remove(i);
	
}
myCarousel.reset();
myCarousel.size(<?php echo count($images); ?>);
<?php $j = 0;foreach ($images as $key=>$image) : ?>
myCarousel.add(<?php echo $j?>,"<li><a href=\"<?php echo  $image['big_image']; ?>\" class=\"cloud-zoom-gallery\" title=\"<?php echo  $image['label'] ?>\" rel=\"useZoom: 'cloud_zoom', smallImage:'<?php echo  $image['image']; ?>'\"><img src=\"<?php echo  $image['thumb'] ?>\" data-srcX2=\"<?php echo  $image['thumb'] ?>\" width=\"<?php echo $thumbImageWidth; ?>\" height=\"<?php echo $thumbImageHeight; ?>\" alt=\"<?php echo  $image['label'] ?>\" /></a></li>");
<?php $j++; endforeach; ?>
